...
const addNumbers = (a: number, b: number = 20 ) => 
{
    return a + b;
}
...
function LoggingString_Constructor(element : string)
{
...
function LoggingTemplate(template : string , hookId: string)
{
        return function(constructor : any)//Unknown means that we don't bother about this function name , but just use it
        //constructor doesn't really matter in parameter . Only for invoking class properties we use this . Also can use Unknown function name , try using unknown and execute 
        {
            const e = new constructor()
                const hookId_element = document.getElementById(hookId);
                if(hookId_element)
                {
                    hookId_element.innerHTML = template;
                }
                const h = document.getElementById("insert_classProperties") as HTMLInputElement;
...
function LogPriceDetails_property(target : any , description : string )
{
...
function LogPriceDetails_method(target1 : any , methodName1 : string , description : PropertyDescriptor ) 
{
...
function LogPriceDetails_parameter(target2 : any , Argument_presentInMethodNAme : string | Symbol , ArgumentPosition : number ) 
{
...
//     Output :-
//     {constructor: ƒ, CalculateTax: ƒ}
// CalculateTax: ƒ CalculateTax()
// GetPriceValue: (...)
// constructor: class TaxDetails
// get GetPriceValue: ƒ GetPriceValue()
// [[Prototype]]: Object
//tax
    public tax = 0;
   ...
   constructor(public price : number)
   {
    ...
    set SetPriceValue(@LogPriceDetails_parameter value : number)
    {
        if (value) {
            this.price = value;            
        }
...
function AutoBind(target? : any, methodName? : string , descriptor? : PropertyDescriptor)
{
    //target : any, methodName : string , descriptor : PropertyDescriptor we are not interested in these values , but just accept them 
        let originalMethod_valueObtained = descriptor?.value;

        let ModifyDescriptor : PropertyDescriptor = 
        {
            enumerable : true,
            configurable : true,
...
const button_element = document.getElementById("clickMe") as HTMLInputElement
...
const fees_variable = new Course_fees("React framework",45);
        console.log(fees_variable);//Course_fees {price: 45, title: 'React framework'}




const form_element = document.getElementById("validation_form") as HTMLFormElement;

form_element.addEventListener("submit",function(event)
{
        event.preventDefault();

        const title_element = document.getElementById("Course_title") as HTMLInputElement;
        const price_element = <HTMLInputElement>document.getElementById("Price_title");

        const title = title_element.value;
        const price = +price_element.value;
...
var formElement_random = document.getElementById("enterRandomValuesIntoHTMLPage") as HTMLFormElement;

formElement_random.addEventListener("submit",function(eventRandomObject)
{
    eventRandomObject.preventDefault();

    var h1element = document.getElementById("h2RandomHeader") as HTMLHeadingElement;
